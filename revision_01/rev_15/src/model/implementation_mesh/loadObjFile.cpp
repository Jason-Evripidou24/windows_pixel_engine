...
)
{
    ...
    for(size_t i = 1; i + 1 < face_vertices.size(); i++)
    {
        ...
        {
            new_triangle.m_v0.m_position = vertex_positions_and_colors[v0_pos_index - 1].first;
        }
        else if( (v0_pos_index < 0) && (v0_pos_index + vertex_positions_and_colors.size() >= 0) )
        {
            new_triangle.m_v0.m_position = vertex_positions_and_colors[vertex_positions_and_colors.size() + v0_pos_index].first;
        }

        if( (v0_pos_tex_coord_index > 0) && (v0_pos_tex_coord_index <= vertex_tex_coords.size()) )
        {
            new_triangle.m_v0.m_tex_coords = vertex_tex_coords[v0_pos_tex_coord_index - 1];
        }
        else if( (v0_pos_tex_coord_index < 0) && (v0_pos_tex_coord_index + vertex_tex_coords.size() >= 0) )
        {
            new_triangle.m_v0.m_tex_coords = vertex_tex_coords[vertex_tex_coords.size() + v0_pos_tex_coord_index];
        }

        if( (v0_normal_index > 0) && (v0_normal_index <= vertex_normals.size()) )
        {
            new_triangle.m_v0.m_normal = vertex_normals[v0_normal_index - 1];
        }
        else if( (v0_normal_index < 0) && (v0_normal_index + vertex_normals.size() >= 0) )
        {
            new_triangle.m_v0.m_normal = vertex_normals[vertex_normals.size() + v0_normal_index];
        }

        if( (v0_color_index > 0) && (v0_color_index <= vertex_positions_and_colors.size()) )
        {
            new_triangle.m_v0.m_color = vertex_positions_and_colors[v0_color_index - 1].second;
        }
        else if( (v0_color_index < 0) && (v0_color_index + vertex_positions_and_colors.size() >= 0) )
        {
            new_triangle.m_v0.m_color = vertex_positions_and_colors[vertex_positions_and_colors.size() + v0_color_index].second;
        }
        //-----------------------------------------------------------------------------------------------------------------//
        ...
        {
            new_triangle.m_v1.m_position = vertex_positions_and_colors[v1_pos_index - 1].first;
        }
        else if( (v1_pos_index < 0) && (v1_pos_index + vertex_positions_and_colors.size() >= 0) )
        {
            new_triangle.m_v1.m_position = vertex_positions_and_colors[vertex_positions_and_colors.size() + v1_pos_index].first;
        }

        if( (v1_pos_tex_coord_index > 0) && (v1_pos_tex_coord_index <= vertex_tex_coords.size()) )
        {
            new_triangle.m_v1.m_tex_coords = vertex_tex_coords[v1_pos_tex_coord_index - 1];
        }
        else if( (v1_pos_tex_coord_index < 0) && (v1_pos_tex_coord_index + vertex_tex_coords.size() >= 0) )
        {
            new_triangle.m_v1.m_tex_coords = vertex_tex_coords[vertex_tex_coords.size() + v1_pos_tex_coord_index];
        }

        if( (v1_normal_index > 0) && (v1_normal_index <= vertex_normals.size()) )
        {
            new_triangle.m_v1.m_normal = vertex_normals[v1_normal_index - 1];
        }
        else if( (v1_normal_index < 0) && (v1_normal_index + vertex_normals.size() >= 0) )
        {
            new_triangle.m_v1.m_normal = vertex_normals[vertex_normals.size() + v1_normal_index];
        }

        if( (v1_color_index > 0) && (v1_color_index <= vertex_positions_and_colors.size()) )
        {
            new_triangle.m_v1.m_color = vertex_positions_and_colors[v1_color_index - 1].second;
        }
        else if( (v1_color_index < 0) && (v1_color_index + vertex_positions_and_colors.size() >= 0) )
        {
            new_triangle.m_v1.m_color = vertex_positions_and_colors[vertex_positions_and_colors.size() + v1_color_index].second;
        }
        //-----------------------------------------------------------------------------------------------------------------//
        ...
        {
            new_triangle.m_v2.m_position = vertex_positions_and_colors[v2_pos_index - 1].first;
        }
        else if( (v2_pos_index < 0) && (v2_pos_index + vertex_positions_and_colors.size() >= 0) )
        {
            new_triangle.m_v2.m_position = vertex_positions_and_colors[vertex_positions_and_colors.size() + v2_pos_index].first;
        }

        if( (v2_pos_tex_coord_index > 0) && (v2_pos_tex_coord_index <= vertex_tex_coords.size()) )
        {
            new_triangle.m_v2.m_tex_coords = vertex_tex_coords[v2_pos_tex_coord_index - 1];
        }
        else if( (v2_pos_tex_coord_index < 0) && (v2_pos_tex_coord_index + vertex_tex_coords.size() >= 0) )
        {
            new_triangle.m_v2.m_tex_coords = vertex_tex_coords[vertex_tex_coords.size() + v2_pos_tex_coord_index];
        }

        if( (v2_normal_index > 0) && (v2_normal_index <= vertex_normals.size()) )
        {
            new_triangle.m_v2.m_normal = vertex_normals[v2_normal_index - 1];
        }
        else if( (v2_normal_index < 0) && (v2_normal_index + vertex_normals.size() >= 0) )
        {
            new_triangle.m_v2.m_normal = vertex_normals[vertex_normals.size() + v2_normal_index];
        }

        if( (v2_color_index > 0) && (v2_color_index <= vertex_positions_and_colors.size()) )
        {
            new_triangle.m_v2.m_color = vertex_positions_and_colors[v2_color_index - 1].second;
        }
        else if( (v2_color_index < 0) && (v2_color_index + vertex_positions_and_colors.size() >= 0) )
        {
            new_triangle.m_v2.m_color = vertex_positions_and_colors[vertex_positions_and_colors.size() + v2_color_index].second;
        }
        //-----------------------------------------------------------------------------------------------------------------//
