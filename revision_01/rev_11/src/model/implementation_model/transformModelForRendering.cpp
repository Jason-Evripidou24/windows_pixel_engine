...
std::queue<MaterialTriangle> Model::transformModelForRendering(const Math::Mat4_f& projection_view_matrix) const
{
    ...
    for(int i = 0; i < m_mesh->m_material_triangles.size(); i++)
    {
        Math::Triangle triangle = m_mesh->m_material_triangles[i].m_triangle;
        const Material& material = m_mesh->m_material_triangles[i].m_material;
