...
// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //
Math::Vertex Math::perspectiveDivideVertex(const Math::Vertex& vertex)
{
    Math::Vertex output = vertex;

    float w;
    float inv_w;

    w = vertex.m_position.m_data[3];
    if(w <= 0.0f)
    {
        output.m_position.m_data[0] = 0.0f;
        output.m_position.m_data[1] = 0.0f;
        output.m_position.m_data[2] = 0.0f;
        output.m_position.m_data[3] = 0.0f;
    }
    else
    {
        inv_w = 1.0f / w;

        output.m_position.m_data[0] = vertex.m_position.m_data[0] * inv_w;
        output.m_position.m_data[1] = vertex.m_position.m_data[1] * inv_w;
        output.m_position.m_data[2] = vertex.m_position.m_data[2] * inv_w;
        output.m_position.m_data[3] = vertex.m_position.m_data[3];
    }
