...
void Renderer::drawModel(Model& model, const Math::Mat4_f& projection_view_matrix, bool draw_filled, float color_mix)
{
    ...
    for(size_t i = 0; i < model.m_mesh->m_sub_meshes.size(); i++)
    {
        ...
        {
            model.m_polygon_buffers.m_buffer0[i][j].resize(model.m_mesh->m_sub_meshes[i].m_polygons[j].size());

            for(size_t k = 0; k < model.m_mesh->m_sub_meshes[i].m_polygons[j].size(); k++)
            {
                Math::transformVertex
                (
                    model.m_polygon_buffers.m_buffer0[i][j][k],
                    model.m_mesh->m_sub_meshes[i].m_polygons[j][k],
            ...
            for(size_t k = 0; k < model.m_polygon_buffers.m_buffer0[i][j].size(); k++)
            {
                if(Utils::checkFloatEquals(model.m_polygon_buffers.m_buffer0[i][j][k].m_position.m_data[3], 0.0f))
                {
                    model.m_polygon_buffers.m_buffer0[i][j].clear();
                    break;
                }
                model.m_polygon_buffers.m_buffer0[i][j][k].perspectiveDivide();
            }
