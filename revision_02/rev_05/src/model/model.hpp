// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //
#ifndef MODEL_HPP
#define MODEL_HPP
// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //


// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //
//-------------------------------------------------------------------------------------------------------------------------//
// Standard library.
//-------------------------------------------------------------------------------------------------------------------------//
#include <queue>
#include <vector>
//-------------------------------------------------------------------------------------------------------------------------//

//-------------------------------------------------------------------------------------------------------------------------//
// Third party.
//-------------------------------------------------------------------------------------------------------------------------//
//-------------------------------------------------------------------------------------------------------------------------//

//-------------------------------------------------------------------------------------------------------------------------//
// Internal.
//-------------------------------------------------------------------------------------------------------------------------//
#include "mesh.hpp"
#include "material_triangle.hpp"

#include "../math/math.hpp"
#include "../math/mat4_f.hpp"
#include "../math/triangle.hpp"
#include "../math/vec3_f.hpp"
//-------------------------------------------------------------------------------------------------------------------------//
// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //


// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //
struct PolygonBuffers
{
    std::vector<std::vector<std::vector<Math::Vertex>>> m_buffer0;
    std::vector<std::vector<std::vector<Math::Vertex>>> m_buffer1;
};

struct Model
{
    //---------------------------------------------------------------------------------------------------------------------//
    Mesh* m_mesh;

    Math::Vec3_f m_position;
    Math::Vec3_f m_scale;
    float m_rotate_rad;
    Math::Vec3_f m_rotate_axis;

    PolygonBuffers m_polygon_buffers;
    //---------------------------------------------------------------------------------------------------------------------//

    //---------------------------------------------------------------------------------------------------------------------//
    Model()
    {
        m_mesh = nullptr;
        m_position = Math::Vec3_f();
        m_scale = Math::Vec3_f();
        m_rotate_rad = 0.0f;
        m_rotate_axis = Math::Vec3_f();
    }

    Model(Mesh* mesh)
    {
        m_mesh = mesh;
        m_position = Math::Vec3_f();
        m_scale = Math::Vec3_f();
        m_rotate_rad = 0.0f;
        m_rotate_axis = Math::Vec3_f();

        //-----------------------------------------------------------------------------------------------------------------//
        size_t num_sub_meshes = m_mesh->m_sub_meshes.size();

        m_polygon_buffers.m_buffer0.resize(num_sub_meshes);
        m_polygon_buffers.m_buffer1.resize(num_sub_meshes);

        for(size_t i = 0; i < num_sub_meshes; i++)
        {
            size_t polygon_count = m_mesh->m_sub_meshes[i].m_polygons.size();

            m_polygon_buffers.m_buffer0[i].resize(polygon_count);
            m_polygon_buffers.m_buffer1[i].resize(polygon_count);

            for(size_t j = 0; j < polygon_count; j++)
            {
                size_t vertex_count = m_mesh->m_sub_meshes[i].m_polygons[j].size();

                m_polygon_buffers.m_buffer0[i][j].reserve(vertex_count + 6);
                m_polygon_buffers.m_buffer0[i][j].resize(vertex_count);

                m_polygon_buffers.m_buffer1[i][j].reserve(vertex_count + 6);
                m_polygon_buffers.m_buffer1[i][j].resize(vertex_count);
            }
        }
        //-----------------------------------------------------------------------------------------------------------------//
    }
    //---------------------------------------------------------------------------------------------------------------------//

    //---------------------------------------------------------------------------------------------------------------------//
    Math::Mat4_f calcModelMatrix() const;
    //---------------------------------------------------------------------------------------------------------------------//
};
// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //


// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //
#endif
// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //