...
-   color_mix is 0.0f <= color_mix <= 1.0f where 0 is 100% color from triangle vertex colors and 1 is 100% color from
    material.
*/
void TileRenderer::fillTriangle
(
...
)
{
    ...
    // Colors.
    //-----------------------------------------------------------------------------------------------------------------//
    uint32_t v0_color = Math::Core::convertVec4fToColor(v0->m_color);
    uint32_t v1_color = Math::Core::convertVec4fToColor(v1->m_color);
    uint32_t v2_color = Math::Core::convertVec4fToColor(v2->m_color);
    //-----------------------------------------------------------------------------------------------------------------//
    ...
    if(Math::Core::checkFloatEquals(area, 0.0f)) { return; } // Accept both clockwise and anti clockwise.
