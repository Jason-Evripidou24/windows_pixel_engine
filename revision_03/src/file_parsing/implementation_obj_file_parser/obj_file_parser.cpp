...
// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //
Math::Geometry::MultiWireframe ObjFileParser::loadMultiWireframe(const std::string& file_folder, const std::string& filename)
{
    Math::Geometry::MultiWireframe multi_wireframe;
    ...
    Math::Geometry::Wireframe curr_wireframe;
    ...
    while(std::getline(file, line))
    {
        ...
        else if(prefix == "usemtl")
        {
            ...
            curr_wireframe = Math::Geometry::Wireframe();
            curr_wireframe.clear();
        }
        //-----------------------------------------------------------------------------------------------------------------//
...
// ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### ##### //
std::vector<RenderWireframe> ObjFileParser::loadRenderWireframes
(
    const std::string& file_folder,
    const std::string& filename
)
{
    std::vector<RenderWireframe> render_wireframes;
    ...
    std::ifstream file(file_folder + filename);
    if(!file.is_open()) { return render_wireframes; }
    //---------------------------------------------------------------------------------------------------------------------//

    RenderWireframe curr_render_wireframe;
    curr_render_wireframe.m_wireframe.clear();
    curr_render_wireframe.m_wireframe_material_name.clear();
    ...
    while(std::getline(file, line))
    {
        ...
        {
            if(curr_render_wireframe.m_wireframe.m_num_polygons > 0)
            {
                render_wireframes.push_back(curr_render_wireframe);
            }

            curr_render_wireframe.m_wireframe.clear();
            curr_render_wireframe.m_wireframe_material_name.clear();
            ss >> curr_render_wireframe.m_wireframe_material_name;
        }
        //-----------------------------------------------------------------------------------------------------------------//
        ...
        else if(prefix == "f")
        {
            ...
            curr_render_wireframe.m_wireframe.addPolygon(new_polygon);
        }
        //-----------------------------------------------------------------------------------------------------------------//
    }
