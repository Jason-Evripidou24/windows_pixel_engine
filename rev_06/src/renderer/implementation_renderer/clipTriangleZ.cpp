...
{
    std::vector<Math::Triangle> clipped_to_min_z = this->clipTriangleMinZ(triangle, min_z_plane);

    std::vector<Math::Triangle> result;
    for(const Math::Triangle& triangle_clip_min : clipped_to_min_z)
    {
        std::vector<Math::Triangle> clipped_to_min_max_z = this->clipTriangleMaxZ(triangle_clip_min, max_z_plane);

        for(const Math::Triangle& triangle_clip_min_max : clipped_to_min_max_z)
        {
